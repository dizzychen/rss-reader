...
Use this file when `~/engineer/` is missing or empty, or when the user wants engineering preferences to persist between
sessions.
...
- whenever they ask for specs, trade-offs, failure analysis, validation plans, root cause analysis, or changeover
  planning
...
Before creating local memory files for this skill, ask for permission and explain that you will keep only compact
engineering context.
...
If the user already asked for a concrete recommendation, help immediately and gather missing context only where it
changes the answer.
